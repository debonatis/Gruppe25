...
import Entitys.ApplicationaccessPK;
 ...
 */
@Stateless
@Path("entitys.applicationaccess")
public class ApplicationaccessFacadeREST extends AbstractFacade<Applicationaccess> {
    ...
    private ApplicationaccessPK getPrimaryKey(PathSegment pathSegment) {
         ...
         * URI path part is supposed to be in form of 'somePath;username=usernameValue;applicationname=applicationnameValue'.
         * Here 'somePath' is a result of getPath() method invocation and
         * it is ignored in the following code.
         * Matrix parameters are used as field names to build a primary key instance.
         */
        Entitys.ApplicationaccessPK key = new Entitys.ApplicationaccessPK();
        javax.ws.rs.core.MultivaluedMap<String, String> map = pathSegment.getMatrixParameters();
        java.util.List<String> username = map.get("username");
        if (username != null && !username.isEmpty()) {
            key.setUsername(username.get(0));
        }
        java.util.List<String> applicationname = map.get("applicationname");
        if (applicationname != null && !applicationname.isEmpty()) {
            key.setApplicationname(applicationname.get(0));
        }
        return key;
    }
    ...
    public void remove(@PathParam("id") PathSegment id) {
        Entitys.ApplicationaccessPK key = getPrimaryKey(id);
